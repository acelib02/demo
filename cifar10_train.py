...
class Train(object):
    ...
    def train(self):
        ...
        #train_dir = './ckpt/'
        if FLAGS.round == '':
            rounddir = time.strftime("%m%d_%H%M%S", time.localtime())
        else:
            rounddir = FLAGS.round
        train_dir = FLAGS.ckpt + '/' + rounddir
        # print(train_dir)
        ...
        for step in range(FLAGS.train_steps):
            ...
            if step + 1 == FLAGS.train_steps:
                ...
                self.latest_run = checkpoint_path + '-' + str(step)
                print('checkpoint saved in %s'%checkpoint_path)
    ...
    def test(self, test_image_array, latest_run=''):
        '''
        ...
        if latest_run!='':
            test_ckpt_path = latest_run
        elif self.latest_run!='':
            test_ckpt_path = self.latest_run
        elif FLAGS.round!='':
            test_ckpt_path = FLAGS.ckpt + '/' + FLAGS.round + '/' + FLAGS.test_ckpt_path
        else:
            print('checkpoint_path set wrong, check the parameter settings.')
            return 0
